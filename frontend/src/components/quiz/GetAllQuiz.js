    ...
    return (
        <section className="container">
            <div className="row mt-5">
                <div className="col-md-6 mb-2 md-mb-0" style={{ color: "#091057" }}>
                    <h4>All Quiz Questions</h4>
                </div>
                <div className="col-md-4 d-flex justify-content-end">
                    <Link to={"/create-quiz"} className="btn btn-outline-primary">
                        <FaPlus /> Add Question
                    </Link>
                </div>
            </div>
            ...
            {questions.map((question, index) => (
                <div key={question.id}>
                    <h4 style={{ color: "#091057" }}>{`${index + 1}. ${question.question}`}</h4>
                    <ul>
                        {question.choices.map((choice, index) => (
                            <li key={index} style={{ color: "#024CAA" }}>{choice}</li>
                        ))}
                    </ul>
